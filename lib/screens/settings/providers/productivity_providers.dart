...
class ExportNotifier extends StateNotifier<ExportState> {
  ...
  Future<void> runExport(ExportRequest request) async {
    ...
    try {
      ...
      final fileNameBase =
          'expanse_export_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> shareLastExport() async {
    ...
    await SharePlus.instance.share(
      ShareParams(
        ...
        text: 'Expanse export: ${file.fileName}',
      ),
    );
  }
...
class BackupState {
  ...
  final String? lastBackupPayload;
  final String? lastBackupFileTempPath;

  const BackupState({
    this.isLoading = false,
    this.error,
    this.backups = const [],
    this.lastBackupPayload,
    this.lastBackupFileTempPath,
  });
  ...
  BackupState copyWith({
    ...
    String? lastBackupPayload,
    String? lastBackupFileTempPath,
  }) {
    return BackupState(
      isLoading: isLoading ?? this.isLoading,
      error: error,
      backups: backups ?? this.backups,
      lastBackupPayload: lastBackupPayload ?? this.lastBackupPayload,
      lastBackupFileTempPath:
          lastBackupFileTempPath ?? this.lastBackupFileTempPath,
    );
  }
}
...
class BackupNotifier extends StateNotifier<BackupState> {
  ...
  Future<void> createBackup() async {
    ...
    try {
      ...
      final file = await _backupFileService.buildBackupFile(snapshot: snapshot);
      final tempPath = await _backupFileService.persistTempBackupBytes(
      ...
      state = state.copyWith(
        isLoading: false,
        backups: backups,
        lastBackupPayload: file.preview,
        lastBackupFileTempPath: tempPath,
      );
    } catch (e) {
